window.addEventListener('load', function () {
    ...
    class Game {
        ...
        update(deltaTime) {
            if (this.enemyTimer > this.enemyInterval) {
                this.enemies = this.enemies.filter(object => object.stillOnScreen);
                this.#addNewEnemy();
                this.enemyTimer = 0;
    ...
    class Spider extends Enemy {
        constructor(game) {
            ...
            this.maxLength = Math.random() * this.height * 0.8;
        }
        update(deltaTime){
            super.update(deltaTime);
            if (this.y < 0 - this.height * 2) this.stillOnScreen = false;
            this.y += this.speedY * deltaTime;
            if (this.y > 400 - 10 * this.speedY) this.speedY *= -1;
        }
        draw(ctx){
            ctx.beginPath();
            ctx.moveTo(this.x + this.width * 0.5, 0);
            ctx.lineTo(this.x + this.width * 0.5, this.y + 20);
            ctx.stroke();
            super.draw(ctx);            
        }
    }
